...
// BEGIN robert_lcc@asus.com.tw for booting ringtone
#ifdef RINGTONE_THREAD
#include <fcntl.h>

#include "BootRingtone.h"
#include "Utility.h"
#endif
// END
...
{
    //add by yangjiajun@wind-mobi.com 2017-10-10 begin
    if(argc == 2 && !strncmp("lcdtest", argv[1], 7)){
        mylcdtest();
        ALOGW("mylcdtest end!");
        return 0;
    }
    ...
    if (!noBootAnimation) {
        ...
        // BEGIN robert_lcc@asus.com.tw
        #ifdef RINGTONE_THREAD
            if(getRingtoneRequest())
            {
                ALOGW("[BootAnimation] start BootRingtone()");
                sp<BootRingtone> bootring = new BootRingtone();
            }
        #endif
        // END
